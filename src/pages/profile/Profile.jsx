...
const Profile = () => {
  ...
  const [follow, setFollow] = useState(
    userSession.following.includes(Number(idUser)) ? true : false
  );
  ...
  const patchFollowers = async (idUser, idProfile) => {
    const newCurrentUser = {
      ...currentUser,
      followers: [...currentUser.followers, idUser],
    };
    const newUserSession = {
      ...userSession,
      following: [...userSession.following, idProfile],
    };
  ...
  const patchUnfollows = async (idUser, idProfile) => {
    const newCurrentUser = {
      ...currentUser,
      followers: currentUser.followers.filter((id) => id !== idUser),
    };
    const newUserSession = {
      ...userSession,
      following: userSession.following.filter((id) => id !== idProfile),
    };
    await patchInfoUser(idUser, newUserSession);
    await patchInfoUser(idProfile, newCurrentUser);
    const user = await getInfoUser(idUser);
  ...
  useEffect(() => {
    if (currentUser) {
      if (follow) {
        if (
          !currentUser.followers.includes(userSession.id) &&
          !userSession.following.includes(currentUser.id)
        ) {
          console.log("hola");
      ...
      {currentUser ? (
        <section className="profile">
          <section className="profile__portrait">
            <figure
              className="profile__portrait__backButton"
              onClick={returnFeed}
            >
              <img src="/images/arrow-left.svg" alt="arrow-left" />
            </figure>
            <figure className="profile__portrait__picture">
              <img src={currentUser.portrait} alt="user_portrait" />
            </figure>
            {isUserSession && (
              <figure
                className="profile__portrait__optionButton"
            ...
            {optionMenu && (
              <ul className="profile__editor">
                <li className="profile__editor__item" onClick={handleEdit}>
                  Edit profile
                </li>
                <li className="profile__editor__item" onClick={logout}>
                  LogOut
                </li>
              </ul>
            )}
          ...
          <section className="profile__header">
            <article className="profile__stats">
              <div className="profile__stats__container profile__stats__followers">
                {" "}
                <span className="profile__stats__number">
              ...
              <div className="profile__stats__container profile__stats__likes">
                <span className="profile__stats__number">
            ...
            <div className="profile__inf">
              ...
              <p className="profile__inf__description">
          ...
          <section className="profile__body">
            <section className="profile__typepost">
              <div
                className={`profile__typepost__button ${
                  typeMedia == "photos" && "profile__typepost__actived"
                }`}
                onClick={() => handleTypePost("photos")}
              >
                Photos
              </div>
              <div
                className={`profile__typepost__button ${
                  typeMedia == "videos" && "profile__typepost__actived"
                }`}
                onClick={() => handleTypePost("videos")}
              >
                Videos
              </div>
              <div
                className={`profile__typepost__button ${
                  typeMedia == "tag" && "profile__typepost__actived"
                }`}
                onClick={() => handleTypePost("tag")}
              >
                Tag
              </div>
            </section>
