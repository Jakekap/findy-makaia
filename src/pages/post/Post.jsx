...
const Post = () => {
  ...
  useEffect(() => {
    if (showContainer) {
      let newCommentsArray =
        postContent.comments.map((element) => element.id) || [];
  ...
  const handleLike = async () => {
    ...
    if (userInfo.likesStore.includes(newPostId)) {
      const findIndexInPost = postLikes.filter(
        (postElement) => postElement !== userInfo.id
      );
      postLikes = findIndexInPost;
      const findIndexInUser = likesStore.filter(
        (userElement) => userElement !== newPostId
      );
      ...
      {showContainer && showContainer2 ? (
        <main className="post__container">
          <figure className="post__figure">
            <img
              src={postContent.media}
              alt="imagen del post"
              className="post__figure__media"
            />
            ...
            <div className="post__figure__info">
              <figure className="post__figure__info__owner">
                {usersInfo.map(
                  (element, index) =>
                    element.id === postContent.userId && (
                      <img
                        key={index}
                        src={element.avatar}
                        alt="miniatura del perfil"
                      />
                    )
                )}
              </figure>
              <p>
                {usersInfo.map(
                  (element) =>
                    element.id === postContent.userId && element.username
                )}
              </p>
              <div className="post__figure__info__extra">
                <figure
                  className="home-feed__icon-container"
                  onClick={handleLike}
                >
                  ...
                  <figcaption className="home-feed__icon-description">
          ...
          <section className="post__description">
            ...
            <article className="post__description__write">
              ...
              <form
                ...
                className="post__description__write__form"
              >
                <input
                  type="text"
                ...
                <button type="submit">
                  {" "}
                  <img src={sendAlt} alt="prueba boton" />{" "}
                </button>
              </form>
            </article>
              ...
              {postContent.comments.map((invCom, index) => (
                <div key={index} className="comments__box">
                  ...
                  <div className="comments__box__description">
                    <p className="description__user">
                      {usersInfo.map(
                        (indUser) =>
                          indUser.id === invCom.userId && indUser.username
                      )}
                    </p>
