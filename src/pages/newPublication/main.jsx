...
const NewPublication = () => {
  ...
  useEffect(() => {
    ...
    if (formContent) {
      if (
        formContent.includes("png") ||
        formContent.includes("Png") ||
        formContent.includes("img") ||
        formContent.includes("Img") ||
        formContent.includes("jpg") ||
        formContent.includes("Jpg") ||
        formContent.includes("jpeg") ||
        formContent.includes("Jpeg") ||
        formContent.includes("image") ||
        formContent.includes("Image") ||
        formContent.includes("img") ||
        formContent.includes("Img")
      ) {
        ...
        formContent.includes("mp4") ||
        formContent.includes("avi") ||
        formContent.includes("flv") ||
        formContent.includes("youtube")
      ) {
  ...
  const postToServer = async (obj) => {
    ...
    if (status === 201) {
      Swal.fire(`Se ha publicado con exito`, "", "success").then(() => {
        navigate(-1);
      });
    } else {
      Swal.fire("Ooopss!", "Hubo un error en la publicacion!", "error");
    }
  };
  ...
  return (
    <main className="publication__container">
      <header className="header">
        ...
        {showContainer ? (
          <p className="header__share__activated" onClick={onShare}>
            Compartir
          </p>
        ) : (
          <p className="header__share">Compartir</p>
        )}
      </header>
      ...
      <form onSubmit={handleSubmit()} className="form">
        <div className="form__box">
          ...
          <input
            ...
            type="url"
            placeholder="Enlace de la imagen o video"
            {...register("pubType")}
          />
        </div>
        <div className="form__box">
        ...
        <div className="form__box">
          ...
          <input
            className="form__box__input"
            type="text"
            placeholder="Etiqueta a tus amigos por su username"
            {...register("pubTags")}
          />
        </div>
      </form>
